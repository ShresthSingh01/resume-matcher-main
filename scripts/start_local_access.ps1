...
if (-not (Test-Path ".\tools\ngrok.exe")) {
    Write-Error "ngrok.exe not found in tools directory!"
    exit 1
}
...
tunnels:
  frontend:
    ...
    proto: http
"@
Set-Content -Path ".\tools\ngrok-tunnels.yml" -Value $ngrokConfigFileContent
...
Write-Host "4. READ docs\PRESENTATION_SCRIPT.md for verification steps!" -ForegroundColor Cyan
Start-Process "docs\PRESENTATION_SCRIPT.md"
